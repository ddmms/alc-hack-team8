...
- Security: injection, shell execution risks, unsafe deserialization.
- Scientific computing: unit conversion mistakes (cm^-1 vs meV vs THz), dimensional consistency in Debye-Waller exponents.
- Linear algebra & FFT: non-Hermitian matrix products, missing FFT normalization (1/N, dt), array broadcasting bugs.
- Edge cases: zero frequencies, empty trajectories, missing chemical cross-sections (sigma_i).
- Error handling: silent catches, swallowed errors.
- Performance: unvectorized Python loops over millions of trajectory steps.
...
**STOP at this step.** Do NOT commit. Wait for the user's literal commit instruction.

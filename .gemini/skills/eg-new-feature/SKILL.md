...
This workflow builds features for the MD-to-INS scientific simulation package (`md_ins`).
...
Verify feature aligns with proposal roadmap (trajectory ingestion, pDOS/tensor extraction, quantum corrections, 1-phonon cross section, multiphonon FFT convolution, or spectrometer kinematics).

Keep modules focused: pure numerical methods decoupled from file I/O.

## Step 1: Write the design doc

Print the design doc to the user. For substantial features, propose writing it to `docs/specs/<feature-slug>.md` using `write_file` and ask the user before creating the file.
...
- Surfaces touched: <files / modules / functions / schemas>
- Interfaces: <function signatures, argument types, return types, dataclasses>
- Units & dimensions: <explicit units: frequencies (cm^-1, meV), lengths (Å), masses (amu), Q (Å^-1)>
- Numerical methods: <FFT grids, windowing, convolution algorithm, tolerances>
- Failure modes: <what happens on degenerate modes, zero frequencies, invalid coordinates>
- Verification criteria: <unit tests, benchmark comparisons against Euphonic/MDANSE>
- Out-of-scope follow-ups: <noted, not built>
```

Cite any relevant PRD from `docs/prds/` or section in `openspec/changes/md-ins-methods/proposal.md`.
...
```
<<<COMPREHENSION_START>>>
You are a fresh reader with no prior context. Below is a design doc for a feature in the md_ins: Inelastic Neutron Scattering simulation from Molecular Dynamics trajectories via intermediate dynamical representations repo. Do NOT critique it yet. Your job is to verify the doc reads clearly to someone who walks in cold.
...
```
<<<DESIGN_START>>>
You are a fresh reviewer with no prior context. Below is a design doc for a feature in the md_ins: Inelastic Neutron Scattering simulation from Molecular Dynamics trajectories via intermediate dynamical representations repo (GEMINI.md at the repo root has the architecture).
...
- Are unit conventions strictly defined across components?
- Are tensor symmetry and positive semi-definiteness preserved?
- Are edge cases (zero frequency, divergent Bose factor at omega -> 0) handled?
- Are there project-specific gotchas the doc ignores? GEMINI.md is your reference.

Check against specification in openspec/ if applicable.
...
```
<<<READINESS_START>>>
You are a fresh implementer with no prior context. Below is a design doc for a feature in the md_ins: Inelastic Neutron Scattering simulation from Molecular Dynamics trajectories via intermediate dynamical representations repo. Imagine you've been told: "Implement this. First pass. No follow-up questions allowed." Could you?
...
1. Core mathematics / physics functions (`md_ins/physics/` or `md_ins/intermediate/`).
2. Data ingestion and trajectory wrappers (`md_ins/trajectory/`).
3. Instrument kinematics and convolution pipelines (`md_ins/instruments/`).
4. Unit and integration tests (`tests/`).
...
- Run `uv run pytest tests/test_<layer>.py` after implementing each layer.
- Ensure detailed balance and quantum zero-point scaling are verified.
...
**STOP.** Do NOT commit. Wait for the user's literal commit instruction.
